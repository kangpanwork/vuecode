...
<div id="mds">
  ...
  <el-table id="mdsTableData" :data="tableDateTemp"  border style="width: 100%" :height="height">
...
import axios from 'axios'
  ...
  methods: {
    // 自适应高度
     getAutoHeight() {
      let el = document.querySelector("#mds")
      if (el) {
    ...
    getMds: function () {
      let url = 'http://localhost:8028/api/bean/getMdsJsonView'
    ...
    getFilters: function () {
      let url = 'http://localhost:8028/api/bean/getMdsFilters'
      axios.get(url).then(response => {
        if (response.status === 200) {
